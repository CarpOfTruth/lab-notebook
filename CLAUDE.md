...
- **Branch**: always work on `dev`. The local app runs against `dev`. **Never push directly to `main`** (no direct commits, no force-push). This is a single-user repo; `main` is kept up to date by merging `dev` → `main` through a PR whenever the owner asks to sync main.
- **PRs**: feature/fix PRs use `--base dev` (`gh pr create --base dev ...`). The only PR into `main` is the dev sync: `gh pr create --base main --head dev ...`, then `gh pr merge <n> --merge` (merge commit; keep `dev`). A PreToolUse hook denies `gh pr create --base main` unless `--head dev` is given.
